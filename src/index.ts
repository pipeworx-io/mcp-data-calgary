...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'DataCalgary');
}
...
async function socrataGet(path: string, apiKey?: string): Promise<unknown> {
  ...
  if (res.status === 403) {
    ...
    if (/invalid app_token/i.test(body)) {
      throw new Error(
        'upstream_throttled: data.calgary.ca refused this call with "Invalid app_token specified". That is Socrata\'s throttle response, not a credential problem — the token is valid and retrying usually succeeds. Socrata meters per token across every portal it hosts, so calling several city packs at once shares one budget. Space the calls out and retry.',
      );
    }
  }
  if (!res.ok) throw await httpError(res, 'data.calgary.ca');
  return res.json();
}
...
async function sfDatasets(query: string | undefined, limit: number | undefined, offset: number | undefined, apiKey?: string) {
  ...
  const res = await pwFetch(`https://api.us.socrata.com/api/catalog/v1?${p}`, { headers: headers(apiKey) });
