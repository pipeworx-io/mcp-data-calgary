...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/calgary_recent \
  -H 'Content-Type: application/json' \
  -d '{"dataset":"311"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/calgary_recent`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "data-calgary": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-data-calgary"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-data-calgary
```
